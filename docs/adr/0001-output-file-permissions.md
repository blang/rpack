...
**Superseded:** the exact-mode contract below was replaced while rpack is in `0.x`. rpack now manages only each file's executable intent; legacy `mode`/`chmod` values remain accepted but are reduced to owner-execute, creation is umask-aware, and the lockfile records canonical `644`/`755` strings compared by owner-execute. The decision text is preserved unchanged as the historical rationale for the exact-mode design, its lockfile format, and the drift it caused under non-`022` umasks.
